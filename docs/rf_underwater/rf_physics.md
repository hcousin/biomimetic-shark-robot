...
Die charakteristische Eindringtiefe heisst **Skin-Tiefe δ**:
...
| Süsswasser (See) | ~100     | ~0.01   |
| Brakwasser       | ~10      | ~0.1    |
| Meerwasser       | ~0.2     | ~5      |

---

## Frequenz-Vergleich (Süsswasser, ρ=100 Ω·m)

| Frequenz | δ [m] | @0.5m [dB] | @1.0m [dB] | @2.0m [dB] | Urteil |
|---|---|---|---|---|---|
...
Bei 433 MHz in Süsswasser: δ = 0.24 m
...
Spezifikation:
  ...
  Adern:      2× Strom (GND + 5V) + 2× Daten (RS485)
  Protokoll:  RS485 / UART, 115200 Baud
  Kosten:     ~15 CHF
  Reichweite: bis 1200 m (RS485 Spec)

Vorteile:
  - Zuverlässig bei jeder Tiefe
  - Niedrige Latenz (<1 ms)
  - Keine RF-Genehmigungen
...
Skin-Tiefe: 0.97 m in Süsswasser
@1.0m:      −9 dB → 35% Signalstärke (grenzwertig)
@1.5m:      −13.5 dB → 21% Signalstärke (unzuverlässig)

Antenne:    λ/4 = 2.77 m Drahtantenne
            oder Ferrit-Kernantennen (verkürzt)

Empfehlung: Nur für <0.8 m in Süsswasser zuverlässig
```
...
```
Architektur:
  OBERFLÄCHE (0–0.2m):  WiFi UDP → volles Kommando
  TAUCHPHASE (>0.2m):   Vorprogrammierte Mission
  
Ablauf:
  1. WiFi: Setze Zieltiefe + Kurs
  2. Hai taucht → WiFi Signal reisst ab
...
Vorteil:    Echtzeitkommunikation bis 100m Tiefe
Nachteil:   Kosten, Komplexität, Latenz ~50ms
```
...
**Für 1–2 m in Süsswasser (Pool/See):**
...
Für Hybrid-Ansatz benötigt die Firmware:
...
- Einfaches ASCII-Protokoll: `T0.5` = Tiefe 0.5m, `F0.6` = Throttle 60%
